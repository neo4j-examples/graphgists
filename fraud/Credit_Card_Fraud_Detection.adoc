...
=== Typical Scenario
In December 2013, the Abington Police arrested two Post Office employees for stealing credit cards and using it to buy more than $50,000 worth of merchandise. Their scheme is quite typical, here is how they operated :

* the Post Office clerks copied the credit card information of some of the customer they served ;

* they looked up the customers to get their address ;

* they would make orders online for goods or gift cards and have them delivered at their victims' home address ;

* an accomplice would post at the address and try to intercept the deliveries ;
...
Now we want to find the common denominator. Is there a common merchant in all of these seemingly innocuous transactions? We just have to tweak the Cypher query to sort out the previous results according to the number of occurrences of each merchant.
[source,cypher]
----
MATCH (victim:person)-[r:HAS_BOUGHT_AT]->(merchant)
WHERE r.status = "Disputed"
