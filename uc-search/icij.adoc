...
A second descriptive query shows us the types of _CLIENT_ and _OFFICER_ relationships present in the example dataset. Note the presence of http://en.wikipedia.org/wiki/Cr%C3%A9dit_Agricole[CrÃ©dit Agricole], the largest retail banking group in France, which is a client of the Singapore-based Sefren Trust managed by Antwerp-based entrepreneur Luscha Baumwald.
...
=== Should we check for companies on the 7Â½th floor? 
